...
def remap_pair_mixed_batch(batch, offset_assets=None):
    if offset_assets is None:
...
def remap_pair_mixed_instance_batch(batch):
    ...
    remap_pair_mixed_batch(batch, offset_assets=offset_assets)
...
def point_collate_fn(batch, mix_prob=0):
    ...
    if random.random() < mix_prob:
        if "instance" in batch:
            remap_pair_mixed_instance_batch(batch)
        else:
            remap_pair_mixed_batch(batch)
